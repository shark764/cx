...
export const defaultForecastFormValues = (memoScenariosOptions: any = []) => {
  ...
  const initIntradayStart = DateTime.now().startOf('day');
  const initIntradayEnd = initIntradayStart.minus({days: (12 * 7) - 1});
  ...
  return ({
    ...
    dayCurveDateRange: [{ startDate: initIntradayStart, endDate: initIntradayEnd }],
    algorithm: 'prophet',
