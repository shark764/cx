...
const Container = styled.div<StyleProps>`
  ...
  width: ${( { size } ) => size}px;
`;
const SvgG: any = styled.g<StyleProps>`
  ${( { fillColor, primary, secondary, theme } ) => ( fillColor
  && css`
        color: ${fillColor};
        fill: ${fillColor};
      `)
...
export function QuestionMark( {
  onClick, size = 25, fill, primary = false, secondary = false, className, ...rest
}: any ) {
  return (
    <Container size={ size } className={ className } onClick={ onClick } { ...rest }>
